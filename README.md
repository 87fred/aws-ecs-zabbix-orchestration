...
* **Isolamento de Banco:** Banco de dados inacessível externamente e sem associação de IP público.
* **Injeção de Segredos:** Injeção dinâmica de segredos no boot da Task através do parâmetro `secrets` do ECS.
* **Mínimo Acesso de Rede:** Comunicação restrita horizontalmente através de regras de Security Groups isoladas (`aws_security_group_rule`).
